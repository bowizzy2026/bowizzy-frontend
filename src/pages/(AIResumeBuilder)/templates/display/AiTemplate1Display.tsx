  ...
  return (
    <div style={{ width: '210mm', minHeight: '297mm', fontFamily: "Raleway, sans-serif", background: '#fff', padding: '32px 40px' }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: 4 }}>
        ...
        {linkedin && <p style={{ fontSize: 9, color: '#555', margin: '2px 0 0' }}>{linkedin}</p>}
      </div>
      <div style={divider} />
      ...
      {personal.aboutCareerObjective && (
        <>
          ...
          <div style={divider} />
          <p style={{ fontSize: 10, color: '#333', lineHeight: 1.5, margin: 0 }}>{htmlToLines(personal.aboutCareerObjective).join(' ')}</p>
        </>
      )}
      ...
      {education.higherEducation.filter(e => e.enabled).map((edu: any, i) => (
        <div key={i} style={{ marginBottom: 6 }}>
          ...
          <p style={{ fontSize: 10, color: '#555', margin: 0 }}>{edu.instituteName} | {fmtYear(edu.startYear)} – {edu.currentlyPursuing ? 'Present' : fmtYear(edu.endYear)}</p>
          {edu.resultFormat && edu.result && <p style={{ fontSize: 10, color: '#333', margin: 0 }}>{edu.resultFormat}: {edu.result}</p>}
        </div>
      ))}
      {education.preUniversityEnabled && education.preUniversity?.instituteName && (
        <div style={{ marginBottom: 6 }}>
          <strong style={{ fontSize: 11 }}>Pre University (12th)</strong>
          <p style={{ fontSize: 10, color: '#555', margin: 0 }}>{education.preUniversity.instituteName} | {fmtYear(education.preUniversity.yearOfPassing)}</p>
        </div>
      )}
      {education.sslcEnabled && education.sslc?.instituteName && (
        <div style={{ marginBottom: 6 }}>
          <strong style={{ fontSize: 11 }}>SSLC (10th)</strong>
          <p style={{ fontSize: 10, color: '#555', margin: 0 }}>{education.sslc.instituteName} | {fmtYear(education.sslc.yearOfPassing)}</p>
        </div>
      )}
