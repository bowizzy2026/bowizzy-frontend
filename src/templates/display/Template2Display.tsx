...
export const Template2Display: React.FC<Template2DisplayProps> = ({ 
  ...
  return (
    <div className="w-[210mm] bg-white" style={{ minHeight: '297mm', paddingTop: '2px', paddingBottom: '2px' }}>
      {/* Header */}
      <div className="border-b border-gray-300 pb-4 mb-6" style={{ padding: '20mm 20mm 0 20mm', marginTop: '2px' }}>
        <h1 className="text-4xl font-bold text-gray-800 tracking-wider" style={{ fontSize: '36px', letterSpacing: '4px' }}>
      ...
      <div className="flex gap-8" style={{ padding: '0 20mm 20mm 20mm' }}>
        ...
        <div className="w-1/3">
          ...
          <div className="resume-section mb-6">
            <h2 className="text-sm font-bold text-gray-800 mb-3" style={{ fontSize: '13px', letterSpacing: '1px' }}>
              CONTACT
            </h2>
            <div className="space-y-2 text-gray-600" style={{ fontSize: '10px' }}>
              ...
              {skillsLinks.links.portfolioEnabled && skillsLinks.links.portfolioUrl && (
                <div className="flex items-center gap-2">
                  <span className="text-gray-800">🌐</span>
                  <span className="break-all">{skillsLinks.links.portfolioUrl}</span>
                </div>
              )}
            </div>
          </div>
            ...
            <div className="resume-section mb-6">
              ...
              <div className="space-y-1">
                {skillsLinks.skills.filter(s => s.enabled && s.skillName).map((skill, idx) => (
                  <div key={idx} className="skill-item flex items-center gap-2">
                    <span className="text-gray-800" style={{ fontSize: '10px' }}>➔</span>
                    <span className="text-gray-600" style={{ fontSize: '10px' }}>{skill.skillName}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="w-2/3">
          ...
          {personal.aboutCareerObjective && (
            <div className="resume-section mb-6">
              ...
              <p className="text-gray-600 leading-relaxed text-justify" style={{ fontSize: '10px' }}>
                {personal.aboutCareerObjective}
              </p>
            </div>
          )}
            ...
            <div className="resume-section mb-6">
              ...
              {experience.workExperiences.filter(exp => exp.enabled).map((exp, idx) => (
                <div key={idx} className="work-item mb-4">
                  ...
                  <p className="text-gray-600 italic" style={{ fontSize: '10px' }}>
                    {exp.companyName} ({exp.startDate} - {exp.currentlyWorking ? 'Present' : exp.endDate})
                  </p>
                  {exp.description && (
                    <p className="mt-2 text-gray-600 leading-relaxed text-justify" style={{ fontSize: '10px' }}>
                      {exp.description}
                    </p>
                  )}
                </div>
            ...
            <div className="resume-section mb-6">
              ...
              {projects.filter(p => p.enabled && p.projectTitle).map((project, idx) => (
                <div key={idx} className="project-item mb-4">
                  ...
                  {project.description && (
                    <p className="mt-1 text-gray-600 leading-relaxed text-justify" style={{ fontSize: '10px' }}>
                      {project.description}
                    </p>
                  )}
                </div>
